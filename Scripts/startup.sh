...
function wait_for_services() {
    ...
    /app/Scripts/wait-for-it.sh db:5432 -- echo "FactionDB Started..";

    >&2 echo "Waiting for RabbitMQ to start..."
    /app/Scripts/wait-for-it.sh mq:5672 -- echo "RabbitMQ Started..";
}

function dotnet_run_watch() {
    wait_for_services;
    echo "Running Build-Service-Dotnet and watching for changes...";
    dotnet watch run;
}
...
function dotnet_run_published() {
    ...
    dotnet run /app/out/dotnet-build.dll;
}
